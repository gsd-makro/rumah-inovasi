        ...
        <div class="modal-content">
            ...
            <form action="{{ route('menus.update', $subSubMenu->id) }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="updateSubSubMenuName{{ $subSubMenu->id }}" class="form-label">Nama Sub Submenu</label>
                        <input type="text" class="form-control" id="updateSubSubMenuName{{ $subSubMenu->id }}"
                            name="name" value="{{ $subSubMenu->name }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="updateSubSubMenuTitle{{ $subSubMenu->id }}" class="form-label">Judul Sub Submenu</label>
                        <input type="text" class="form-control" id="updateSubSubMenuTitle{{ $subSubMenu->id }}"
                            name="title" value="{{ $subSubMenu->title }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="updateSubSubMenuUrl{{ $subSubMenu->id }}" class="form-label">Url</label>
                        <input type="text" class="form-control" id="updateSubSubMenuUrl{{ $subSubMenu->id }}"
                            name="url" value="{{ $subSubMenu->url }}">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="updateSubSubMenuActive{{ $subSubMenu->id }}"
                                name="is_active" {{ $subSubMenu->is_active ? 'checked' : '' }}>
                            <label class="form-check-label" for="updateSubSubMenuActive{{ $subSubMenu->id }}">Aktif</label>
                        </div>
                    </div>
                    <!-- Order Dropdown -->
                    <div class="mb-3">
                        <label for="updateSubSubMenuOrder{{ $subSubMenu->id }}" class="form-label">Urutan Menu (Saat Ini: {{ $subSubMenu->order }})</label>
                        <select class="form-select" id="updateSubSubMenuOrder{{ $subSubMenu->id }}" name="order">
                            @foreach ($subMenu->children as $menu)
                                <option value="{{ $menu->order }}" {{ $menu->order == $subSubMenu->order ? 'selected' : '' }}>
                                    Urutan {{ $menu->order }} {{ $menu->order == $subSubMenu->order ? '(Saat Ini)' : '' }}
                                </option>
                            @endforeach
                        </select>
