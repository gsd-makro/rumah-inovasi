<div class="modal fade" id="update{{ $subMenu->id }}" tabindex="-1" aria-labelledby="updateSubLabel{{ $subMenu->id }}"
    aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('menus.update', $subMenu->id) }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="updateSubSubMenuTitle{{ $subMenu->id }}" class="form-label">Judul Sub
                            Submenu</label>
                        ...
                        <select name="content_type" class="form-select" id="menuContentType">
                            ...
                            @foreach ($contentTypes as $contentType)
                                <option value="{{ $contentType }}"
                                    {{ $contentType == $subMenu->content_type ? 'selected' : '' }}>
                                    {{ ucfirst($contentType) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox"
                                id="updateSubSubMenuActive{{ $subMenu->id }}" name="is_active"
                                {{ $subMenu->is_active ? 'checked' : '' }}>
                            <label class="form-check-label"
                                for="updateSubSubMenuActive{{ $subMenu->id }}">Aktif</label>
                        </div>
                    </div>
                    <!-- Order Dropdown -->
                    <div class="mb-3">
                        <label for="updateSubSubMenuOrder{{ $subMenu->id }}" class="form-label">Urutan Menu (Saat Ini:
                            {{ $subMenu->order }})</label>
                        <select class="form-select" id="updateSubSubMenuOrder{{ $subMenu->id }}" name="order">
                            @foreach ($menu->children as $menu)
                                <option value="{{ $menu->order }}"
                                    {{ $menu->order == $subMenu->order ? 'selected' : '' }}>
                                    Urutan {{ $menu->order }}
                                    {{ $menu->order == $subMenu->order ? '(Saat Ini)' : '' }}
                                </option>
                            @endforeach
                        </select>
